...
with tf.Graph().as_default():
    ...
    label_tensor = tf.placeholder(tf.float32, [None, 20, 2], name='labels')
    ...
    with tf.variable_scope("RPN-reg"):
        ...
        classifier = tf.reshape(classifier, (tf.shape(classifier)[0], 20,2))
    ...
    with tf.name_scope("cross_entropy") as scope:
        cross_entropy = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(labels=label_tensor, logits=classifier))
    ...
    with tf.name_scope("accuracy") as scope:
        correct_prediction = tf.equal(
            tf.argmax(classifier, 1), tf.argmax(label_tensor, 1))
